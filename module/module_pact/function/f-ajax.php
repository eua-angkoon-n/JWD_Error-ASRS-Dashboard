...
Class Dashboard {
    ...
    private $conveyor;
    private $stv;
    private $sql_conveyor;
    public function __construct($date, $interval){
        $d = explode("||//", $date);
        $this->start    = $d[0]." 00:00:00";
        $this->end      = $d[1]." 23:59:59";
        $this->interval = $interval;
        $this->crane    = Setting::$PACTMachine['crane'];
        $this->conveyor = Setting::$PACTMachine['conveyor'];
        $this->stv      = Setting::$PACTMachine['stv'];
    }
    ...
    private function err_count($q){
        ...
        if($q){
            if(is_array($q)){
                ...
                $this->sql_conveyor = $m = implode(' OR ', $conditions);
                $sql .= "AND ( $m ) ";
            } else {
    ...
    private function err_chart(){
        ...
        $sql .= "    WHEN $this->sql_conveyor THEN 'conveyor' ";
    ...
    private function getIntervalQuery(){
        switch($this->interval){
            case 'hour':
                ...
                $sql .= "COUNT(CASE WHEN $this->sql_conveyor THEN 1 ELSE NULL END) AS conveyor, ";
            ...
            case 'day':
                ...
                case 'week':
                    $sql  = "SELECT 
                                ...
                                COUNT(CASE WHEN $this->sql_conveyor THEN 1 ELSE NULL END) AS conveyor,
